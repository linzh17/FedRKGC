...
class KGEModel(nn.Module):
    def __init__(self, args, model_name):
        ...
        self.layer = nn.Linear(768*2,768)
        nn.init.xavier_uniform_(self.layer.weight)

    def forward(self, sample, relation_embedding, entity_embedding, neg=True):
        if relation_embedding.shape[1] == 2*entity_embedding.shape[1]:
            relation_embedding = self.layer(relation_embedding)
        
        if not neg:
            #torch.index_select 根据索引选取tensor中相对应的元素
            head = torch.index_select(
                entity_embedding,
                dim=0,
                #sample[:,0] 选出的是头实体对应的id号
                index=sample[:, 0]
            ).unsqueeze(1)
            ...
            if tail_part == None:
                #测试时 采用测试的全部节点 
                #print(entity_embedding.unsqueeze(0).shape)1*14541*256 14541 节点数量 256 嵌入长度
...
class KGERelModel(nn.Module):
    def __init__(self, args, model_name):
        super(KGERelModel, self).__init__()
        #print("kgerel")
        self.model_name = model_name
        self.embedding_range = torch.Tensor([(args.gamma + args.epsilon) / args.hidden_dim])
        self.gamma = nn.Parameter(
    ...
    def forward(self, sample, relation_embedding, entity_embedding, neg=True):
        ...
        if not neg:
            ...
            if tail_part == None:
                #测试时 采用测试的全部节点 
                #print(entity_embedding.unsqueeze(0).shape)1*14541*256 14541 节点数量 256 嵌入长度
                #print(relation_embedding.unsqueeze(0).shape)
                relation = relation_embedding.unsqueeze(0)
            else:
                negative_sample_size = tail_part.size(1)
                tail = torch.index_select(
                    entity_embedding,
